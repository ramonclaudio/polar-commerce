...
export const createCheckoutSession = action({
  ...
  handler: async (ctx, args) => {
    ...
    if (userId) {
      ...
      cart = await ctx.runQuery(internal.cart.internal_getCartByUserId, {
        userId,
      });
    } else if (args.sessionId) {
      console.log('[Checkout] Looking up cart by sessionId:', args.sessionId);
      // @ts-ignore - Type instantiation is excessively deep (known Convex issue)
      cart = await ctx.runQuery(internal.cart.internal_getCartBySessionId, {
    ...
    const cartItemsRaw = await ctx.runQuery(
      internal.cart.internal_getCartItems,
      {
        cartId: cart._id,
      },
    );
    ...
    if (userId) {
      ...
      if (!customerEmail) {
        const user = await ctx.runQuery(internal.cart.internal_getAuthUser, {
          userId,
        });
        if (user) {
    ...
    try {
      ...
      await ctx.runMutation(internal.cart.internal_updateCartCheckout, {
        cartId: cart._id,
...
export const handleCheckoutSuccess = action({
  ...
  handler: async (ctx, { checkoutId }) => {
    ...
    try {
      ...
      await ctx.runMutation(internal.cart.internal_createOrder, {
        checkoutId,
        userId,
        email: customerEmail,
      ...
      if (checkout.status === 'succeeded' && cartId) {
          ...
          if (productId && quantity) {
            try {
              await ctx.runMutation(
                internal.products.decrementInventoryInternal,
                {
        ...
        await ctx.runMutation(internal.cart.internal_clearCartItems, {
          cartId,
        });
